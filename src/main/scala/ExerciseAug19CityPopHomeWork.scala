object ExerciseAug19CityPopHomeWork extends App {
  ...
  // percentage: Int - yearly growth rate in percentages
  ...
  // it is little bit tricky because we do not want to use return statements
  // we want to return tha last line only
   ...
   */
  def getCityYear(p0: Double, percentage: Double, delta: Double, targetPopulation: Int) = {
    9000 //FIXME

    var calculatedPopulation: Double = p0
    var targetYear: Int = 0
    while (calculatedPopulation < targetPopulation && targetYear >= 0) {

      calculatedPopulation = (calculatedPopulation * (1 + percentage / 100) + delta)
      targetYear = targetYear + 1

      if (calculatedPopulation < p0) targetYear = -1
    }
    targetYear
  }
